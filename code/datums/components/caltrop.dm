...
/*Toxic sludge burns your feet NOT WORKING =(
/datum/component/caltrop/sludge

/datum/component/caltrop/sludge/Initialize(_min_damage = 0, _max_damage = 0, _probability = 100,  _flags = NONE)
	min_damage = _min_damage
	max_damage = max(_min_damage, _max_damage)
	probability = _probability
	flags = _flags

	RegisterSignal(parent, list(COMSIG_MOVABLE_CROSSED), .proc/sludging)
//all this just to have it make burn instead of brute damage
/datum/component/caltrop/sludge/proc/sludging(datum/source, atom/movable/AM)
	var/atom/A = parent
	if(!A.has_gravity())
		return
	...
	if(ishuman(AM))
		...
		if(HAS_TRAIT(H, TRAIT_LIGHT_STEP))
			damage *= 0.75
		H.apply_damage(damage, BURN, picked_def_zone, wound_bonus = CANT_WOUND)
