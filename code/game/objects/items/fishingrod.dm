...
/obj/item/fishingrod/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	if(!istype(target, /turf/open/water) && !istype(target, /turf/open/indestructible/ground/outside/river) && !istype(target, /turf/open/transparent/openspace/outside/fishingspot))
		return ..()
